...
default_job = st.session_state.get("bridge_job_selected", job_keys[0])
with st.form("bridge_job_form"):
    job_key = st.selectbox(
        ...
        key="bridge_job_input",
    )
    bridge_apply = st.form_submit_button("Apply Job")
if bridge_apply:
    st.session_state["bridge_job_selected"] = job_key
job_key = st.session_state.get("bridge_job_selected", job_key)
